...
%------------------
	properties
   	...
   	methods
		function obj = init(obj,varargin)
			...
			% - Check inputs
			if ~isempty(A.sim) % sim
				if ~strcmp(A.sim,'peru') & ~strcmp(A.sim,'pacmed');
					disp(' ');
					disp('sim must be peru or pacmed');
					disp(' '); return
				end	
			end
			...
			sim = [];
			if isempty(A.sim)
                        	sim = input('---------------------------\nperu_chile(1) or pacmed(2)\n---------------------------\n>> ');
				if ~ismember(sim,[1 2])
					disp(' ');
					disp('Only 1 or 2 allowed');
					disp(' '); return
				end
			end
			if sim == 1
			...
			obj.paths.plots.diag.mldfigs      = [obj.paths.simPath,'diagFigures/',name,'/mld_figs/'];	
         		...
         		obj.grid.lat_rho  = double(ncread(obj.paths.grid,'lat_rho'));
                        obj.grid.lon_psi  = double(ncread(obj.paths.grid,'lon_psi'));
                        obj.grid.lat_psi  = double(ncread(obj.paths.grid,'lat_psi'));
         		...
         		tmp               = load(obj.paths.zcoord);
         		obj.grid.Hz       = tmp.Hz;
         		obj.grid.z_r      = tmp.z_r;
         		obj.grid.z_w      = tmp.z_w; clear tmp

		       	% - get more coord diags 
       		  	obj.grid.nx         = size(obj.grid.lon_rho,1);
         		obj.grid.ny         = size(obj.grid.lon_rho,2);
         		obj.grid.nz         = size(obj.grid.Hz,3);
			tmp                 = ncread(obj.paths.avg,'zeta');
         		obj.grid.nt         = size(tmp,3); clear tmp
		...
		function obj = getBudg(obj,varargin)
                        % --------------------
                        % - Grab parameters for budget
                        % - Define frequency of budget computation (freq)
                        % - or the element/compound (N,NO3,NH4,C,O) to perform budget (varname).
			...
			% Check for optional plots
			if isempty(A.timeplot)
				return
			else
				if max(A.timeplot) > obj.region.nt
					disp('Bad time entry, try again');
					return
				end
				disp(['Plotting budget figures for ',A.varname]);
				plotConc(obj,'time',A.timeplot);
		...
		function obj = getRates(obj);
			% -------------------
			% - Grab 3D production/loss rates based on inputs from getParam
		...
		function obj = getFluxes(obj);
			% -------------------
			% - Grab 2D fluxes based on inputs from getParam
			% - Fluxes in mmol/m2/s
			%
			% - Usage:
			%   obj = getFluxes(obj)
			...
			if strcmp(obj.budget.varname,'N')
				...
				vars = {'FG_N2O','SED_DENITRIF','OTHER_REMIN','FG_N2','FG_N2O_AO1',...
					'FG_N2O_SIDEN','FG_N2O_SODEN','FG_N2O_ATM','FG_N2O_NEV',...
					'POC_SED_LOSS','POC_FLUX_IN'};
			elseif strcmp(obj.budget.varname,'NO3')
		...
		function obj = computeDzDt(obj,varargin)
			% -------------------
			% - Compute height of grid cells from his/avg files
			% - End results is m
			%
			% - Usage:
			%   obj = computDzDt(obj,varargin)
			%
			%   STILL NEED TO CODE VARARGIN INPUTS
			% - Inputs:
			%   depthmin - minimum depth to perform budget
			%   depthmax - maximum depth to perform budget
			%
			% - Example:
			%   obj = computeDzDt(obj,'depthmin',0,'depthmax',inf)
			% -------------------
		...
		function obj = computeDcDt(obj)
			...
			% - Budget to perform is defined in getParam
			% - End result is mmol/m3/s
			%
			% - Usage:
			%   obj = computeDcDt(obj)
			...
			for v = 1:length(vars)
				...
				obj.romsData.(vars{v}).dcdt = (his2 - his1)/obj.budget.dzdt.dt;
			end 
		...
		function obj = computeXYZflux(obj)
			...
			% - Fields are defined in getParam
			% - End result is mmol/m3/s
			%
			% - Usage:
			%   obj = computeXYZflux(obj)
			...
			for f = 1:length(vars)
				...
				for t = 1:obj.region.nt
					...
					adx(1:nx-1,:,:)     = (temp.X(1:nx-1,:,:) - temp.X(2:nx,:,:)) ./ ...
							       obj.region.area3d(1:nx-1,:,:) ./ obj.budget.dzdt.dz(1:nx-1,:,:,t);
					% - Y advection
					ady(1:nx,1:ny,1:nz) = NaN; 		
					ady(:,1:ny-1,:)     = (temp.Y(:,1:ny-1,:) - temp.Y(:,2:ny,:)) ./ ...
							       obj.region.area3d(:,1:ny-1,:) ./ obj.budget.dzdt.dz(:,1:ny-1,:,t);
					% - Z advection
					adz(:,:,:) = (temp.Z(:,:,1:nz) - temp.Z(:,:,2:nz+1)) ./ obj.budget.dzdt.dz(:,:,:,t);
					% - Z diffusion
					dfz(:,:,:) = (temp.Zd(:,:,1:nz) - temp.Zd(:,:,2:nz+1)) ./ obj.budget.dzdt.dz(:,:,:,t);
					% - Apply mask
					adx = adx.*obj.region.mask_rho3d;
					ady = ady.*obj.region.mask_rho3d;
					adz = adz.*obj.region.mask_rho3d;
		...
		function obj = computeSMS(obj)
			...
			for v = 1:length(vars)
				...
				if strcmp('NO3',vars{v})
					% Make SED_DENITRIF 3D
					tmpsed          = zeros(size(obj.romsData.NITROX.data));
					tmpsed(:,:,1,:) = obj.romsData.SED_DENITRIF.data; % Input at bottom 
					tmpsed          = tmpsed ./ obj.budget.dzdt.dz; % Converto mmol/m3/s
					% Get SMS 
				...
				if strcmp('N2O_decomp',vars{v})
					...
					obj.romsData.N2O_ATM.sms    = tmpatm2 - ...              				 
							     	      obj.romsData.N2OATM_CONS.data;     			       
					obj.romsData.N2O_SIDEN.sms  = tmpatm3 - ...              			                
							     	      obj.romsData.N2OSIDEN_CONS.data;   			         
				...
				if strcmp('O2',vars{v})
					% Get SMS
					obj.romsData.O2.sms = obj.romsData.O2_PRODUCTION - obj.romsData.O2_CONSUMPTION
				end
			end
		...
		function obj = intConc(obj)
			...
			for i = 1:length(vars)		
				tmpdata           		= obj.romsData.(vars{i}).data .* obj.budget.dzdt.dz; % mmol/m3/s --> mmol/m2/s
				tmpdata           		= squeeze(nansum(tmpdata,3));
				tmpdata(obj.region.mask_rho==0) = NaN;
		...
		function obj = intBudg(obj)
			...
			% Gather dimensions
			dzdt      = obj.budget.dzdt.dz;
                        tlat      = obj.region.lat_rho;
                        tlon      = obj.region.lon_rho;	
			
			% Go through each variables
			for i = 1:length(vars)
				
				% Grab terms
				dcdt = obj.romsData.(vars{i}).dcdt;
				adx  = obj.romsData.(vars{i}).adx;
				ady  = obj.romsData.(vars{i}).ady;
				adz  = obj.romsData.(vars{i}).adz;
				dfz  = obj.romsData.(vars{i}).dfz;
				sms  = obj.romsData.(vars{i}).sms;
		...
		function plotDecomp(obj,varargin)
			% ------------------
			% - Plot the integrated decomp comparisons
			%
			% - Inputs:
			%   time - time record to plot (default--> all)
			%   prc  - percentile to limit colorbar
			...
			% Get Variables
			if strcmp(obj.budget.varname,'N2O_decomp')
				disp('...nitrous oxide decomposition plots...')
			elseif strcmp(obj.budget.varname,'O2')
				return
			end
			...
			% Compare N2O tracers
			obj.int.NET_N2O       = obj.int.conc.N2O_DECOMP        - obj.int.conc.N2O;        % mmol/m2
			obj.int.NET_DENITRIF3 = obj.int.rates.DENITRIF3_DECOMP - obj.int.rates.DENITRIF3; % mmol/m2/s
			obj.int.NET_FG_N2O    = obj.fluxes.FG_N2O_DECOMP       - obj.fluxes.FG_N2O;       % mmol/m2/s
			obj.int.NET_ADX       = obj.int.adx.N2O_DECOMP         - obj.int.adx.N2O;         % mmol/m2/s
			obj.int.NET_ADY       = obj.int.ady.N2O_DECOMP         - obj.int.ady.N2O;         % mmol/m2/s
			obj.int.NET_ADZ       = obj.int.adz.N2O_DECOMP         - obj.int.adz.N2O;         % mmol/m2/s
			obj.int.NET_DFZ       = obj.int.dfz.N2O_DECOMP         - obj.int.dfz.N2O;         % mmol/m2/s
			obj.int.NET_DCDT      = obj.int.dcdt.N2O_DECOMP        - obj.int.dcdt.N2O;        % mmol/m2/s
			obj.int.NET_SMS       = obj.int.sms.N2O_DECOMP         - obj.int.sms.N2O;         % mmol/m2/s

			% Plot results
			vars  = {'NET_N2O','NET_DENITRIF3','NET_FG_N2O','NET_ADX','NET_ADY','NET_ADZ','NET_DFZ','NET_DCDT','NET_SMS'};
			units = {'mmol N_2O m^{-2}','mmol N_2O m^{-2} s^{-1}','mmol N_2O m^{-2} s^{-1}', ...
				 'mmol N_2O m^{-2} s^{-1}','mmol N_2O m^{-2} s^{-1}','mmol N_2O m^{-2} s^{-1}',...
				 'mmol N_2O m^{-2} s^{-1}','mmol N_2O m^{-2} s^{-1}','mmol N_2O m^{-2} s^{-1}'};
			for i = 1:length(vars)
				% Go through each time-record (or dont, based on input)
				for t = 1:length(A.time)
		...
		function plotBudg(obj,varargin);
			...
			for i = 1:length(vars)
				...
				for t = 1:length(A.time)
					...
					% Gather terms
					dcdt = obj.int.dcdt.(vars{i})(:,:,tt);
					adv  = obj.int.adx.(vars{i})(:,:,tt) + ...
					       obj.int.ady.(vars{i})(:,:,tt) + ...
					       obj.int.adz.(vars{i})(:,:,tt);
					dfz  = obj.int.dfz.(vars{i})(:,:,tt);
					sms  = obj.int.sms.(vars{i})(:,:,tt);
				
					% Blank land
					dcdt(obj.region.mask_rho==0) = NaN;
			 		adv(obj.region.mask_rho==0) = NaN;
					dfz(obj.region.mask_rho==0) = NaN;
					sms(obj.region.mask_rho==0) = NaN;
		...
		function plotResults(obj)
			% ------------------
			% - Bar charts for N2O contributions per tracer
			%
			% - Example:
			%   plotResults(obj)
			% ------------------	
			...
			% Colors
			clrs = [rgb('FireBrick'); rgb('RoyalBlue'); rgb('DarkGreen'); rgb('DarkMagenta')];
			
			% LineWidths
			lnwd = [2];

			% Get frequency of output
			if obj.param.freq_comput == 0
				xstr = ['Days'];
			elseif obj.param.freq_comput == 1
				xstr = ['Months'];
			end

			% Contributions from air-sea fluxes
			for t = 1:obj.region.nt
				airsea.atm(t) = nansum(obj.fluxes.FG_N2O_ATM(:,:,t)   .* obj.region.grid_area,'all');
				airsea.den(t) = nansum(obj.fluxes.FG_N2O_SODEN(:,:,t) .* obj.region.grid_area,'all');
				airsea.bou(t) = nansum(obj.fluxes.FG_N2O_SIDEN(:,:,t) .* obj.region.grid_area,'all');
				airsea.nit(t) = nansum(obj.fluxes.FG_N2O_AO1(:,:,t)   .* obj.region.grid_area,'all');
				produc.atm(t) = nansum(obj.int.sms.N2O_ATM(:,:,t)     .* obj.region.grid_area,'all') - ...
						airsea.atm(t);
				produc.den(t) = nansum(obj.int.sms.N2O_SODEN(:,:,t)   .* obj.region.grid_area,'all') - ...
						airsea.den(t);
				produc.bou(t) = nansum(obj.int.sms.N2O_SIDEN(:,:,t)   .* obj.region.grid_area,'all') - ...
						airsea.bou(t);
				produc.nit(t) = nansum(obj.int.sms.N2O_AO1(:,:,t)     .* obj.region.grid_area,'all') - ...
						airsea.nit(t);
				advect.atm(t) = nansum(obj.int.adx.N2O_ATM(:,:,t)     .* obj.region.grid_area,'all') + ...
						nansum(obj.int.ady.N2O_ATM(:,:,t)     .* obj.region.grid_area,'all') + ...
						nansum(obj.int.adz.N2O_ATM(:,:,t)     .* obj.region.grid_area,'all');
				advect.den(t) = nansum(obj.int.adx.N2O_SODEN(:,:,t)   .* obj.region.grid_area,'all') + ...
						nansum(obj.int.ady.N2O_SODEN(:,:,t)   .* obj.region.grid_area,'all') + ...
						nansum(obj.int.adz.N2O_SODEN(:,:,t)   .* obj.region.grid_area,'all');
				advect.bou(t) = nansum(obj.int.adx.N2O_SIDEN(:,:,t)   .* obj.region.grid_area,'all') + ...
						nansum(obj.int.ady.N2O_SIDEN(:,:,t)   .* obj.region.grid_area,'all') + ...
						nansum(obj.int.adz.N2O_SIDEN(:,:,t)   .* obj.region.grid_area,'all');
				advect.nit(t) = nansum(obj.int.adx.N2O_AO1(:,:,t)     .* obj.region.grid_area,'all') + ...
						nansum(obj.int.ady.N2O_AO1(:,:,t)     .* obj.region.grid_area,'all') + ...
						nansum(obj.int.adz.N2O_AO1(:,:,t)     .* obj.region.grid_area,'all');
			end

			% Get stacked bars for airsea
			airsea.dat      = [];
			airsea.dat(:,1) = [airsea.nit];	
			...
			ylabel('Air-Sea Flux of N_2O (mmol/s)');
			xlabel(xstr)
			legend('AO1','SODEN','SIDEN','ATM');
			...
			fig(1) = piofigs('mfig',1);
			plot(1:obj.region.nt,-airsea.nit,'color',clrs(1,:),'LineWidth',lnwd);
			hold on
			...
			ylabel('Net N_2O from S-M-S (mmol/s)');
			xlabel(xstr)
			legend('AO1','SODEN','SIDEN','ATM');
			...
			fig(1) = piofigs('mfig',1);
			plot(1:obj.region.nt,produc.nit,'color',clrs(1,:),'LineWidth',lnwd);
			hold on
			...
			ylabel('Net Advection of N_2O (mmol/s)');
			xlabel(xstr)
			legend('AO1','SODEN','SIDEN','ATM');
			...
			fig(1) = piofigs('mfig',1);
			plot(1:obj.region.nt,advect.nit,'color',clrs(1,:),'LineWidth',lnwd);
			hold on
			...
			if ~isempty(A.vars) % - user provided inputs, load variables
				for i = 1:length(A.vars)
					...
					elseif typ == 3
						tmpdata 			  = tmpdata(obj.region.lon_lim(1):obj.region.lon_lim(2),...
                                                                 		       	    obj.region.lat_lim(1):obj.region.lat_lim(2),:,:);
						tmpdata(obj.region.mask_rho3d==0) = NaN;
						obj.romsData.(A.vars{i}).data     = tmpdata;
					end
					disp(['...done!']);
					disp('-------------------------');
		...
		function [fig,ax,cb] = plotData(obj,varargin)
			...
			%
			% Usage:
			% - [obj,fig,cb] = plotData(obj,varargin)
			% 
			% Inputs (varargin):
			...
			% - plttype = tmpfigs (default) or define file type ('png' or 'pdf');
			%
			% Outputs:
			% - fig = figure handle(s)
			% - ax  = axes handle(s)
			...
			lonbounds = [floor(obj.grid.minlon_rho) ceil(obj.grid.maxlon_rho)];
			latbounds = [floor(obj.grid.minlat_rho) ceil(obj.grid.maxlat_rho)];
			...
			for i = 1:length(A.vars)
				for j = 1:length(zidx)
					for k = 1:length(A.time);
						...
						% - mask coast
						data(obj.region.mask_rho==0) = NaN;
			...
			for ff = 1:length(A.vars)
				...
				% - Load z_avg variable
				data  = ncread(obj.paths.zavg,A.vars{ff});
		...
		function obj = getMLD(obj,varargin)
			...
			temp = ncread(obj.paths.zavg,'temp');
			salt = ncread(obj.paths.zavg,'salt');
		...
		function obj = cravatteCMPR(obj)
				...
				%for l = 1:12
					...
					for z = 1:lvls
						...
						tmpout{z,l} = F(double(tmp.outlon),double(tmp.outlat));
                                        	tmpout{z,l}(~obj.region.mask_rho) = nan;
					end
     				end
			...
			disp('STILL NEED TO FIX TO REGION');
			return
	
			% - squeeze variables	
			if strcmp(var,'nstar')
 				obj.romsData.(var).surfdata = squeeze(nanmean(ncread(obj.paths.zavg,obj.plots.diag.NO3.roms.var,...
               			    			      [1,1,1,1],[inf,inf,obj.plots.diag.surfNlevs,inf])- 16.* ...
               			    			      ncread(obj.paths.zavg,obj.plots.diag.PO4.roms.var,...
               			    			      [1,1,1,1],[inf,inf,obj.plots.diag.surfNlevs,inf]),3)) + 2.9;
			elseif strcmp(var,'npp')
            			tmp                         = ncread(obj.paths.avg,'TOT_PROD');
            			tmpVol                      = repmat(obj.grid.Hz,1,1,1,12);
            			obj.romsData.(var).surfdata = squeeze(nansum(tmp.*tmpVol,3)).*3600*24*12;
			elseif ismember(var,{'ssh','zws','mws'})
				obj.romsData.(var).surfdata = ncread(obj.paths.avg,obj.plots.diag.(var).roms.var);
				if strcmp(var,'ssh');
			...
			elseif ismember(var,{'ws','wsc'})
				tmpu   = ncread(obj.paths.avg,obj.plots.diag.zws.roms.var);
				tmpv   = ncread(obj.paths.avg,obj.plots.diag.mws.roms.var);
				tmpang = ncread(obj.paths.grid,'angle');
				if strcmp(var,'ws')
					[obj.romsData.ws.surfdata,~]  = obj.WindStress(tmpu,tmpv,obj.grid.lon_rho,obj.grid.lat_rho,tmpang);
				elseif strcmp(var,'wsc')
					[~,obj.romsData.wsc.surfdata] = obj.WindStress(tmpu,tmpv,obj.grid.lon_rho,obj.grid.lat_rho,tmpang);
				end
			else
 		        	obj.romsData.(var).surfdata = squeeze(nanmean(ncread(obj.paths.zavg,obj.plots.diag.(var).roms.var,...
 	                	    			      [1,1,1,1],[inf,inf,obj.plots.diag.surfNlevs,inf]),3));
			end	
				...
				for d = 1:length(A.depth)
					...
					elseif ~isnan(A.depth(d))
						if strcmp(tmpvar,'nstar')==1
							...
							tmpno3   		     = squeeze(ncread(obj.paths.zavg,'NO3',[1 1 zind 1],[inf inf 1 inf]));
							tmppo4   		     = squeeze(ncread(obj.paths.zavg,'PO4',[1 1 zind 1],[inf inf 1 inf]));
							obj.romsData.nstar.zsurfdata = tmpno3 - 16.*tmppo4 + 2.9;
						else
							
							tmpdepth                        = obj.grid.z_avg_dep; %incread(obj.paths.diag.(tmpvar).file,obj.paths.diag.(tmpvar).zvar);
							zind                            = find(tmpdepth == A.depth(d));
							obj.romsData.(tmpvar).zsurfdata = squeeze(ncread(obj.paths.zavg,obj.plots.diag.(tmpvar).roms.var,[1 1 zind 1],[inf inf 1 inf]));
						end
						ddcnt    = ddcnt + 1;
	...
	methods (Static)
		...
		function togridout = gridToRoms(romslon,romslat,romsmask,romsdepthbnds,var,lon,lat,depth,month)
        		% ------------------
			% -  
			% ------------------
        		...
        		nx         = size(romslon,1); 
			ny         = size(romslon,2);
         		nz         = length(romsdepthbnds); nl=12;
         		np         = length(var(:));
         		circ       = 2*pi*earthRadius('km');   
         		lon(lon<0) = lon(lon<0)+360;
         		togrid     = nan(nx,ny,nz,12);
         		togridcnt  = togrid;
         		botid      = sum(romsmask,3); 
         		for i = 1 : np
            			if mod(i,10000)==0
               				i
            			end
            			if depth(i)<romsdepthbnds(2,end)
               				iddepth = find(depth(i)>=romsdepthbnds(1,:) & ...
                  				  depth(i) < romsdepthbnds(2,:));
            			else
               				iddepth = nz;
            			end
            			tmpmask = romsmask(:,:,iddepth);
            			id      = find(romslon(:)>lon(i)-1 &...
            				  romslon(:)<lon(i)+1 & romslat(:)>lat(i)-1 &...
            				  romslat(:)<lat(i)+1 & tmpmask(:));
            			if isempty(id); continue;end
            			romslonfilt = romslon(id);
            			romslatfilt = romslat(id);
            			dist=[];
            			for ii = 1 : length(id)
               				dist(ii) = distance(lon(i),lat(i),romslonfilt(ii),romslatfilt(ii));
            			end
            			[~,idmin] = min(dist);
            			[row,col] = ind2sub([nx,ny],id(idmin));
            			if botid(row,col)<iddepth; iddepth = botid(row,col);end;
            			if isnan(togrid(row,col,iddepth,month(i)))
               				togrid(row,col,iddepth,month(i))    = var(i);
               				togridcnt(row,col,iddepth,month(i)) = 1;
            			else
               				togrid(row,col,iddepth,month(i))    = togrid(row,col,iddepth,month(i)) + var(i);
               				togridcnt(row,col,iddepth,month(i)) = togridcnt(row,col,iddepth,month(i)) +1;
            			end
         		end
         		togridout = togrid./togridcnt;
		end % end static method gridToRoms
