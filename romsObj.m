...
%------------------
	...
	methods
		...
		function [obj] = loadGrid(obj,varargin)
			...
			% Load basic grid
			fields = {'lon_rho','lat_rho','mask_rho','h'};
			for i = 1:length(fields)
				try
					obj.grid.(fields{i}) = double(ncread(obj.paths.grid,fields{i},[obj.grid.rho2D(1,:)],[obj.grid.rho2D(2,:)]));
				catch
					disp([fields{i},' is missing from grid file']);
				end
			end
			...
			if A.full ~= 0
				...
				fields = {'lon_u','lat_u','lon_v','lat_v','pm','pn','angle','mask_u','mask_v'};
				for i = 1:length(fields)
					try
						if strcmp(fields{i},['lon_u','lat_u','mask_u']);
							obj.grid.(fields{i}) = double(ncread(obj.paths.grid,fields{i},[obj.grid.u2D(1,:)],[obj.grid.u2D(2,:)]));
						elseif strcmp(fields{i},['lon_v','lat_v','mask_v']);
							obj.grid.(fields{i}) = double(ncread(obj.paths.grid,fields{i},[obj.grid.v2D(1,:)],[obj.grid.v2D(2,:)]));
						else
							obj.grid.(fields{i}) = double(ncread(obj.paths.grid,fields{i},[obj.grid.rho2D(1,:)],[obj.grid.rho2D(2,:)]));
						end
					catch
		...
		function obj = loadData(obj,vars,file,varargin)
			...
			for i = 1:length(vars)
				...
				strings_to_replace = {'meter second-1' ,'m s$^{-1}$';...
		...
		function obj = computeVar(obj,vars,file,varargin)
			...
			for i = 1:length(vars)
				...
				elseif strcmp(vars{i},'sigma0')
					...
					tmp_salt_abs = romsObj.getSA(tmp_salt,tmp_pres,tmp_lon,tmp_lat);
		...
		function obj = sliceROMS(obj,vars,choice,deg,file,varargin);
			...
			if ~isempty(A.zdep) | strcmp(choice,'lat') | strcmp(choice,'lon')  
				obj = zslice(obj,vars,A.zdep,file,'type',A.type);
				obj.slice = [];
				nz  = length(A.zdep);
				nt  = sum(cell2mat({obj.info.(['phy_',A.type])(file).time})); 
			else
			...
			for ff = 1:length(vars)
				tmpdata{ff} = obj.data.(A.type).(vars{ff}).slice;
			end
			tmpdim = ndims(tmpdata{1});  
			data   = cat(tmpdim+1,tmpdata{:});
			clear tmpdata;
			...
			for i = 1:length(vars)
				...
				[tmpslice{i},tmpmask{i},tmpdepth{i}] = romsObj.lonlat_slice(dims,lonlat,tmpdata,mask,deg,...
					A.zdep,fillmat,i,length(vars));
			end 
			...
			for j = 1:ns
				...
				for ff = 1:length(vars)
					for t = 1:nt
						if ns == 1
							tmp = squeeze(tmpslice(:,:,t,ff));
							tmp(masktmp==0) = NaN;
							tmpslice(:,:,t,ff) = tmp;
						else
							tmp = squeeze(tmpslice(:,:,t,j,ff));
							tmp(masktmp==0) = NaN;
							tmpslice(:,:,t,j,ff) = tmp;
						end
					end
		...
		function obj = ipslice(obj,vars,ip,file,varargin)
			...
			for i = 1:length(vars)
				for t = 1:nt
					...
					tmpvar = obj.data.(A.type).(vars{i}).data(:,:,:,t);
					tmpip  = obj.data.(A.type).sigma0.data(:,:,:,t);
					if strcmp(vars{i},'u')
			...
			obj.slice.ip = ip;
			if nt == 1
				obj.slice.sigma0 = repmat(ip,[lx ly]);
			else
				obj.slice.sigma0 = repmat(ip,[lx ly nt]);
			end
		...
		function obj = computeDcDt(obj,vars,file)
			...
			tmpinfo    = ncinfo([obj.paths.avg,obj.info.avg.files{file}]);
			fieldnames = {tmpinfo.Attributes.Name};
			fieldvalue = {tmpinfo.Attributes.Value};
			...
			for v = 1:length(vars)
				...
				obj.budget.(vars{v}).dcdt = ((his(:,:,:,2:end) - his(:,:,:,1:end-1))/dt) .* obj.grid.avg.mask_rho3d;

				% Mask padding
				obj.budget.(vars{v}).dcdt(1,:,:,:) = nan;
				obj.budget.(vars{v}).dcdt(:,1,:,:) = nan;
		...
		function obj = computeXYZflux(obj,vars,file)
			...
			for i = 1:length(vars)
				...
				% Mask padding
				obj.budget.(vars{i}).adx(1,:,:,:) = nan;
				obj.budget.(vars{i}).ady(1,:,:,:) = nan;
				obj.budget.(vars{i}).adz(1,:,:,:) = nan;
				obj.budget.(vars{i}).dfz(1,:,:,:) = nan;
				obj.budget.(vars{i}).adx(:,1,:,:) = nan;
				obj.budget.(vars{i}).ady(:,1,:,:) = nan;
				obj.budget.(vars{i}).adz(:,1,:,:) = nan;
				obj.budget.(vars{i}).dfz(:,1,:,:) = nan;
		...
		function obj = computeSMS(obj,vars,rates,smseq,titles,varargin)
			...
			% mask padding
			obj.budget.(vars{1}).sms(1,:,:,:) = nan;
			obj.budget.(vars{1}).prod(1,:,:,:) = nan;
			obj.budget.(vars{1}).cons(1,:,:,:) = nan;
			obj.budget.(vars{1}).sms(:,1,:,:) = nan;
			obj.budget.(vars{1}).prod(:,1,:,:) = nan;
			obj.budget.(vars{1}).cons(:,1,:,:) = nan;
		...
		function obj = computeNet(obj,vars,varargin)
			...
			% Mask padding
			obj.budget.(vars{1}).net(1,:,:,:) = NaN;
			obj.budget.(vars{1}).net(1,:,:,:) = NaN;
			obj.budget.(vars{1}).net(:,1,:,:) = NaN;
			obj.budget.(vars{1}).net(:,1,:,:) = NaN;
			obj.budget.(vars{1}).net(end,:,:,:) = NaN;
			obj.budget.(vars{1}).net(end,:,:,:) = NaN;
			obj.budget.(vars{1}).net(:,end,:,:) = NaN;
			obj.budget.(vars{1}).net(:,end,:,:) = NaN;
		...
		function plotTotBudg(obj,vars,varargin);
			...
			% defaults for optional  arguments
			A.time      = [size(obj.budget.(vars{1}).adx,4)];
			A           = parse_pv_pairs(A,varargin); % parse method arguments to A
			...
			for i = 1:length(vars)
				...
				% Get actual net
				tmp.mynet.mean = tmp.dcdt.mean - (tmp.adv.mean + tmp.sms.mean + tmp.fg.mean + tmp.sed.mean + tmp.dfz.mean);
				tmp.mynet.std  = tmp.dcdt.std  - (tmp.adv.std  + tmp.sms.std  + tmp.fg.std  + tmp.sed.std  + tmp.dfz.std );
				terms{end+1} = 'mynet';
				tits{end+1}  = 'MyNet';
		...
		function [fig,cb] = slicePlot(obj,slicedata,t,varargin)
			...
			if isempty(A.levels)
				...
				A.levels = romsObj.prclims(tmpdata(:),'prc',A.prc,'bal',A.bal);
			...
			for t = 1:nt
				...
				for i = 1:dmsn
					for j = 1:ns
						...
						for z = 1:nz
							if dmsn == nx
								tmpslice(i,z,t,j) = interp1(squeeze(lonlat(i,:)),squeeze(tmpdata(i,:,z)),deg(j));
								tmpmask(i,z,t,j)  = interp1(squeeze(lonlat(i,:)),squeeze(mask(i,:,z)),deg(j));
								tmpdepth(i,z,t,j) = depth(z);
							elseif dmsn == ny
								tmpslice(i,z,t,j) = interp1(squeeze(lonlat(:,i)),squeeze(tmpdata(:,i,z)),deg(j));
								tmpmask(i,z,t,j)  = interp1(squeeze(lonlat(:,i)),squeeze(mask(:,i,z)),deg(j));
								tmpdepth(i,z,t,j) = depth(z);
...
function [paths] = getDiagPaths;	
	...
	paths.diag.O2.units = {'mmol O$_2$ m$^{-3}$'};
	...
	paths.diag.PO4.units = {'mmol P m$^{-3}$'};
	...
	paths.diag.SiO3.units = {'mmol Si m$^{-3}$'};
